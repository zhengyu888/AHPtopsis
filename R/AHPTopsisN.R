...
#' AHPTopsisN(normaldata,MatixData)
#' }
AHPTopsisN<-function(data,matrixdata){
  maxdata<-data
  nc<-ncol(maxdata)
  nr<-nrow(maxdata)
  MAXdata=NULL
  ...
  Wi<-AHPWeight(matrixdata)$weightResult


  for (i in 2:nc){
    maxdata1<-maxdata[,c(1,i)]
    maxdata1$Djia<-Wi[i-1]*((maxdata[,i]-BestResult["MAXdata",i-1])**2)
    maxdata2<- maxdata1[,c(1,3)]
    names(maxdata2)[2]<-names(maxdata)[i]
    Dmax<-merge(Dmax,maxdata2)[order(Dmax[,1]),]

    mindata1<-maxdata[,c(1,i)]
    mindata1$Djian<-Wi[i-1]*((maxdata[,i]-BestResult["MINdata",i-1])**2)
